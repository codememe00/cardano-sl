...
\emph{Wallet state}
\begin{align*}
& \mathsf{Wallet} = \mathsf{UTxO} \times \mathsf{Pending} \\
...
& \qquad \text{valid when~} \mathit{ins} \subseteq \dom (\mathsf{available} ~ (utxo, pending))
\\
& \mathsf{applyBlock} ~ b ~ (\mathit{utxo}, \mathit{pending}) \\
& \qquad \text{valid when~} \dom (\mathsf{txouts} ~ b) \cap \dom \mathit{utxo} = \emptyset
\end{align*}
%
...
can only spent outputs in the wallet's current UTxO. Alternative, we could
require that
%
\begin{equation*}
...
We will use Lemma~\ref{lem:balance_props}.\ref{eq:balance_props_union}, about $\mathsf{balance}$
...
Next we apply Lemma~\ref{lem:balance_props}.\ref{eq:balance_props_minus} to distribute $\mathsf{balance}$
...
Applying Lemma~\ref{lem:balance_props}.\ref{eq:balance_props_minus} to distribute $\mathsf{balance}$ over $\subtractdom$ gives us
...
So we can apply Lemma~\ref{lem:balance_props}.\ref{eq:balance_props_union} to distribute $\mathsf{balance}$ over $\cup$) to give us
\begin{equation*}
\begin{split}
