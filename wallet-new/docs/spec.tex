...
& \subseteq u
  \tag{1} \label{eq:utxo_props_restrictdom_subseteq}
\\
  \mathit{ins} \subtractdom u
& \subseteq u
  \tag{2} \label{eq:utxo_props_subtractdom_subseteq}
\\
  u \restrictrange \mathit{outs}
& \subseteq u
  \tag{3} \label{eq:utxo_props_restrictrange_subseteq}
\\
  \mathit{ins} \restrictdom (u \cup v)
& = (\mathit{ins} \restrictdom u) \cup (\mathit{ins} \restrictdom v)
  \tag{4} \label{eq:utxo_props_restrictdom_union}
\\
...
& = \mathit{ins} \restrictdom u
  \tag{6} \label{eq:utxo_props_restrictdom_dom}
\\
  (\mathit{ins} \cap \dom u) \subtractdom u
& = \mathit{ins} \subtractdom u
  \tag{7} \label{eq:utxo_props_subtractdom_dom}
\\
  (\mathit{ins} \cup \dom u) \subtractdom u \cup v
& = (\mathit{ins} \cup \dom u) \subtractdom v
  \tag{8} \label{eq:utxo_props_subtractdom_remove_dom}
\\
  \mathit{ins} \subtractdom u
...
%
\begin{proof}[Proof (Lemma~\ref{lem:utxo_props}.\ref{eq:utxo_props_restrictdom_union})]
\begin{align*}
  & ~ \mathit{ins} \restrictdom (u \cup v) \\
...
& \text{if } \dom u \cap \dom v = \emptyset
  \tag{1} \label{eq:balance_props_minus}
\\
  \mathsf{balance} ~ (\mathit{ins} \subtractdom u)
& = \mathsf{balance} ~ u - \mathsf{balance} ~ (\mathit{ins} \restrictdom u)
  \tag{2} \label{eq:balance_props_union}
\end{align}
%
...
= & ~ (\dom u \cup \mathsf{txins} ~ b) \subtractdom (\mathsf{txouts} ~ b \restrictrange \mathsf{TxOut_{ours}}) & \text{\{ Lemma~\ref{lem:utxo_props}.\ref{eq:utxo_props_subtractdom_remove_dom} \}} \\
...
we add support for rollback to the wallet we cannot assume anymore that
...
\end{equation*}
\label{lemma:dom_available}
\end{lemma}

\begin{proof}
Follows immediately from
Lemma~\ref{lem:utxo_props}.\ref{eq:utxo_props_subtractdom_subseteq}.
\end{proof}
...
\end{equation*}
\label{lemma:change_vs_utxo}
\end{lemma}

\begin{proof}
Follows from Invariant~\ref{inv:change_vs_available}
and Lemma~\ref{lemma:dom_available}.
\end{proof}

Lemma~\ref{lemma:change_vs_utxo} is not very deep. All new transactions should
...
& \mathsf{change} ~ \mathit{pending} \cup \mathsf{available} ~ w = \mathsf{total} ~ w
  \tag{1} \label{eq:total_props_sets} \\
& \mathsf{balance} ~ (\mathsf{change} ~ \mathit{pending}) + \mathsf{balance} ~ (\mathsf{available} ~ w) = \mathsf{balance} ~ (\mathsf{total} ~ w)
  \tag{2} \label{eq:total_props_balance}
\end{align}
\label{lemma:total_props}
\end{lemma}
%
\begin{proof}
(\ref{lemma:total_props}.\ref{eq:total_props_sets}) follows directly from
the definition.
(\ref{lemma:total_props}.\ref{eq:total_props_balance}) follows from
(\ref{lemma:total_props}.\ref{eq:total_props_sets}) and
Lemma~\ref{lem:balance_props}.\ref{eq:balance_props_union} with
...
%
\intertext{Expanding definitions we have}
%
a & = \mathsf{balance} ~ (\mathsf{available} ~ (utxo, pending)) \\
t & = \mathsf{balance} ~ (\mathsf{available} ~ (utxo, pending) \cup \mathsf{change} ~ pending)
%
\intertext{Use Lemma~\ref{lem:balance_props}.\ref{eq:balance_props_union} to
distribute $\mathsf{balance}$ over $\cup$. The lemma's antecedent is satisfied
by Invariant~\ref{inv:change_vs_available}.}
%
a & = \mathsf{balance} ~ (\mathsf{available} ~ (utxo, pending)) \\
t & = \mathsf{balance} ~ (\mathsf{available} ~ (utxo, pending)) \\
  & + \mathsf{balance} ~ (\mathsf{change} ~ pending)
%
\intertext{Expanding definitions and simplifying we get}
%
a & = \mathsf{balance} ~ (\mathsf{txins} ~ pending \subtractdom utxo) \\
t & = a + \mathsf{balance} ~ (\mathsf{change} ~ pending)
%
\intertext{Finally, we apply
Lemma~\ref{lem:balance_props}.\ref{eq:balance_props_minus} to distribute
$\mathsf{balance}$ over $\subtractdom$, to get}
%
...
%
\intertext{Applying Lemma~\ref{lem:balance_props}.\ref{eq:balance_props_minus}
...
Lemma~\ref{lem:balance_props}.\ref{eq:balance_props_union} to distribute
$\mathsf{balance}$ over $\cup$ to give us}
%
...
$\mathsf{applyBlock}_\sigma$ is only defined in terms of the inputs and outputs
of the block, we can easily define a variation $\mathsf{applyBlock}'_\sigma$,
...
Lemma~\ref{lem:utxo_props}.\ref{eq:utxo_props_restrictdom_dom}
%
\begin{equation*}
...
\end{equation*}
%
and by Lemma~\ref{lem:utxo_props}.\ref{eq:utxo_props_subtractdom_dom}
%
\begin{equation*}
...
\todo[inline]{It would be nicer if this proof could be made more formal; it's
tricky because it involves properties of the blockchain.}

Lemma~\ref{lem:prefiltering} justifies the definition of the wallet with
prefiltering (Figure~\ref{fig:wallet_with_prefiltering}).
...
\subsection{Basic rollback model}
...
\mathsf{available} ~ (\mathit{utxo}, \mathit{pending})
   & = \mathsf{txins} ~ \mathit{pending} \subtractdom \mathit{utxo} \tag{unchanged} \\
...
\end{align*}
%
\caption{\label{fig:basic_rollback_model}Basic rollback model}
\end{figure}

The basic rollback is shown in Figure~\ref{fig:basic_rollback_model}. In this
model the wallet state is a list of \emph{checkpoints}, the value of the wallet
at various times throughout its lifetime; each call to $\mathsf{applyBlock}$
introduces a new checkpoint. The initial wallet is the singleton list; We cannot
roll back a wallet that contains only a single checkpoint (a rollback before any
blocks have been applied would anyway not make semantic sense).
...
the next section). Nonetheless, the definition of $\mathsf{available}$ can
...
pending transactions. Note that the precondition to $\mathsf{newPending}$ still
...
track of the ``expected UTxO'': the unspent outputs that arise from
expected transactions. The wallet model that does this is shown in
Figure~\ref{fig:model_with_expected_UTxO}.
...
\begin{invariant}
\begin{equation*}
\mathsf{txins} ~ pending \subseteq \dom (utxo \cup expected \cup \mathsf{txouts} ~ \mathit{pending})
\end{equation*}
...
is: what is the wallet's \emph{minimum balance across all possible futures}?
...
minimizes the wallet's balance, and is self-consistent in the sense that if $t_1
\in S$ and $t_1$ has a known dependency on $t_2$, then $t_2$ also in $S$ (or in
the wallet's UTxO). Note that the absence of some dependency information only
gives us \emph{more} freedom in picking the elements of this set; thus, missing
...
When there are no rollbacks, the set of expected transactions will be empty, and
there can be no pending transactions that depend on other pending  transactions.
This means for example that
Invariant~\ref{inv:txins_in_dom_utxo_expected_pending} collapes to
Invariant~\ref{inv:txins_in_dom_utxo}.
