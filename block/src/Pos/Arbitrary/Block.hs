...
instance (HasProtocolMagic, HasProtocolConstants) => Arbitrary (T.BodyProof T.MainBlockchain) where
    arbitrary = genericArbitrary
    shrink T.MainProof {..} =
...
instance (HasProtocolMagic, HasProtocolConstants) => Arbitrary T.MainToSign where
    arbitrary = genericArbitrary
    shrink = genericShrink
